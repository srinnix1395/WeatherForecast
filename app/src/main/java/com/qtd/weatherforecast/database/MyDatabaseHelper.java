...
public class MyDatabaseHelper extends SQLiteOpenHelper {
    ...
    public CityPlus getCityByID(int idCity) {
        ...
        String CITY_SELECT_QUERY =
                String.format("Select %s.%s,%s,%s,%s.%s,%s.%s,%s,%s,%s.%s from %s,%s where "
                                + DatabaseConstant.TABLE_CITY + "." + DatabaseConstant.ID_CITY + "=" + DatabaseConstant.TABLE_CURRENT_WEATHER + "." + DatabaseConstant.ID_CITY + " and "
                                + DatabaseConstant.TABLE_CITY + "." + DatabaseConstant.ID_CITY + "=" + idCity,
    ...
    public void updateWeatherHour(ArrayList<WeatherHour> weatherHour, int idCity) {
        SQLiteDatabase db = getWritableDatabase();
        db.beginTransaction();
        try {
            ContentValues contentValues = new ContentValues();
            for (int i = 0; i < weatherHour.size(); i++) {
                contentValues.clear();
                contentValues.put(DatabaseConstant.HOUR, weatherHour.get(i).getHour());
                contentValues.put(DatabaseConstant.ICON, weatherHour.get(i).getIcon());
                contentValues.put(DatabaseConstant.TEMP_C, weatherHour.get(i).getTemp());
                contentValues.put(DatabaseConstant.CHANCE_RAIN, Integer.parseInt(weatherHour.get(i).getRain().replace('%', ' ').trim()));

                long result = db.update(DatabaseConstant.TABLE_HOUR, contentValues, DatabaseConstant.ID_CITY + "=" + idCity + " AND " + DatabaseConstant.ORDER + "=" + i, null);
    ...
    public void deleteCity(int id) {
        ...
        db.beginTransaction();
        try {
            db.delete(DatabaseConstant.TABLE_CURRENT_WEATHER, DatabaseConstant.ID_CITY + "=" + id, null);
            db.delete(DatabaseConstant.TABLE_DAY, DatabaseConstant.ID_CITY + "=" + id, null);
            db.delete(DatabaseConstant.TABLE_HOUR, DatabaseConstant.ID_CITY + "=" + id, null);
            db.delete(DatabaseConstant.TABLE_CITY, DatabaseConstant.ID_CITY + "=" + id, null);
            db.setTransactionSuccessful();
        } catch (Exception e) {
